...
import { conditions, ecgPatterns } from "@/db/schema";
...
function parseFeatures(json: string | null): KeyFeature[] {
  ...
  try {
    ...
    if (Array.isArray(parsed))
      return parsed.filter((p) => p?.feature && p?.value);
    return [];
  } catch {
    return [];
  }
        ...
        <View className="px-4 pt-2">
          ...
          <View className="rounded-[32px] border border-border bg-surface p-5">
            <View className="mb-4 h-14 w-14 items-center justify-center rounded-[22px] bg-clinical-redSoft">
              <Activity size={26} color="#FF3B30" strokeWidth={1.5} />
            </View>
          ...
          {features.length > 0 ? (
            <View className="mt-4 rounded-clinical border border-border bg-surface p-4">
              <Text className="mb-3 font-bodySemi text-[11px] uppercase tracking-[1.5px] text-text-tertiary">
                Key Features
              </Text>
              {features.map((f, i) => (
                <View
                ...
                >
                  <Text className="font-bodySemi text-[12px] uppercase tracking-[1.1px] text-text-tertiary flex-shrink-0 w-32">
                    {f.feature}
                  </Text>
