...
export default function ECGListScreen() {
  const { db } = useDatabase();
  const [rows, setRows] = useState<ECGRow[]>([]);
        ...
        <View className="px-4 pt-2">
          ...
          <View className="mb-4 flex-row items-center gap-2 rounded-clinical border border-border bg-surface px-3 py-2">
            ...
            <TextInput
              ...
              placeholder="Search rhythms (e.g. atrial fibrillation)"
              placeholderTextColor="#505058"
              className="flex-1 font-body text-[14px] text-text-primary"
              autoCorrect={false}
              autoCapitalize="none"
              accessibilityLabel="Search ECG patterns"
            />
          </View>
        </View>
