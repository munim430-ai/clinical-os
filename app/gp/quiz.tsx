...
import { conditions, osceCards } from "@/db/schema";
import {
...
> = {
  all: { label: "All stations", icon: Brain, color: "#2470FF" },
  history: { label: "History", icon: GraduationCap, color: "#4499FF" },
  examination: { label: "Examination", icon: Stethoscope, color: "#FFA01D" },
  management: { label: "Management", icon: Pill, color: "#2470FF" },
  "data-interpretation": {
...
export default function QuizScreen() {
  ...
  const filtered = useMemo(
    () =>
      filter === "all"
        ? allCards
        : allCards.filter((c) => c.stationType === filter),
    [allCards, filter],
  );
  ...
  const stationCounts = useMemo(() => {
    ...
    for (const c of allCards) {
      if (c.stationType)
        counts[c.stationType] = (counts[c.stationType] ?? 0) + 1;
    }
    return counts;
  }, [allCards]);
  ...
  function startQuiz() {
    ...
    const pool = [...filtered]
      ...
      .slice(0, Math.min(QUIZ_LENGTH, filtered.length));
    setShuffled(pool);
    setIdx(0);
    setRevealed(false);
    setCorrect(0);
  ...
  function answer(isCorrect: boolean) {
    ...
    if (!isCorrect && card.conditionName) {
      const updated = {
        ...weakAreas,
        [card.conditionName]: (weakAreas[card.conditionName] ?? 0) + 1,
      };
  ...
  return (
    <View className="flex-1 bg-background">
      <View className="px-4 pt-12 pb-3 flex-row items-center">
        ...
        <Text className="ml-3 font-heading text-[24px] text-text-primary">
          Medi-Quiz
        </Text>
      </View>
      ...
      {phase === "menu" && (
        <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 80 }}>
          ...
          <View className="mb-5 rounded-clinical border border-border-accent bg-mint-soft p-5 items-center">
            <Brain size={40} color="#2470FF" strokeWidth={1.6} />
            <Text className="mt-3 font-headingBold text-[18px] text-text-primary">
            ...
            <View className="mt-4 flex-row gap-3">
              <View className="rounded-pill bg-surface px-3 py-1">
                <Text className="font-bodySemi text-[11px] text-mint">
                  Best: {bestScore}/{QUIZ_LENGTH}
                </Text>
              </View>
            </View>
          </View>
            ...
            {(Object.keys(STATION_META) as StationFilter[]).map((key) => {
              ...
              return (
                ...
                >
                  <Icon
                    size={13}
                    color={active ? meta.color : "#8A91A8"}
                    strokeWidth={1.7}
                  />
                  <Text
                    className={[
                  ...
                  <Text
                    className={[
                      "font-body text-[11px]",
                      active ? "text-mint" : "text-text-tertiary",
                    ].join(" ")}
                  >
                    {count}
                  </Text>
          ...
          {topWeakAreas.length > 0 ? (
            <View className="mb-5 rounded-clinical border border-border bg-surface p-4">
              ...
              {topWeakAreas.map(([name, count]) => (
                ...
                >
                  ...
                  <View className="rounded-lg bg-clinical-redSoft px-2 py-0.5">
                    <Text className="font-bodySemi text-[11px] text-clinical-red">
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.85}
            accessibilityRole="button"
            accessibilityLabel={`Start quiz with ${Math.min(
              QUIZ_LENGTH,
              filtered.length,
            )} questions`}
          >
      ...
      {phase === "quiz" && shuffled[idx] ? (
        <View className="flex-1 px-4">
          ...
          <View className="mb-4 mt-2 flex-row items-center">
            ...
            <Text className="ml-3 font-bodySemi text-[12px] text-text-tertiary">
          ...
          <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
            {/* Question card */}
            <View className="mb-4 rounded-clinical border border-border bg-surface p-5">
            ...
            {!revealed ? (
              ...
              >
                <Text className="font-bodySemi text-[14px] text-mint">
                  Reveal answer
                </Text>
              </TouchableOpacity>
            ) : (
              <View>
                <View className="mb-4 rounded-clinical border border-border-accent bg-mint-soft p-5">
                  <Text className="mb-2 font-bodySemi text-[10px] uppercase tracking-[1.3px] text-mint">
                    Answer
                  </Text>
                  <Text className="font-body text-[14px] leading-6 text-text-primary">
                    {shuffled[idx].answer}
                  </Text>
                </View>
                <Text className="mb-3 text-center font-body text-[13px] text-text-tertiary">
        ...
        <ScrollView contentContainerStyle={{ padding: 24, paddingBottom: 56 }}>
          <View className="items-center">
            <Trophy
              size={60}
              color={
                correct >= 7 ? "#2470FF" : correct >= 4 ? "#FFA01D" : "#FF3B30"
              }
              strokeWidth={1.5}
            />
            ...
            <Text className="mt-1 font-body text-[14px] text-text-tertiary">
              {correct >= 8
                ? "Excellent!"
                : correct >= 6
                  ? "Good work!"
          ...
          {missed.length > 0 ? (
            <View className="mt-8">
              ...
              {missed.map((card) => (
                <View
                  key={card.id}
                  className="mb-3 rounded-clinical border border-border bg-surface p-4"
                >
                  {card.conditionName ? (
                    <Text className="mb-1 font-body text-[11px] uppercase tracking-[1px] text-text-tertiary">
                      {card.conditionName}
                    </Text>
                  ) : null}
                  <Text className="mb-2 font-bodySemi text-[13px] text-text-primary">
                    {card.question}
                  </Text>
                  <Text className="font-body text-[12px] leading-5 text-text-secondary">
                    {card.answer}
                  </Text>
                </View>
              ))}
          ...
          >
            ...
            <Text className="font-headingBold text-[15px] text-text-inverse">
              Try again
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
